...
import { getColumns } from "@/components/admin/admin-user-column";

const PAGE_SIZE_OPTIONS = [10, 20, 50];

export default function AdminUsersTable() {
  const { users, usersTotal, fetchUsers, error } = useAdmin();
  ...
  useEffect(() => {
    const timeout = setTimeout(
      () => {
        fetchUsers({ limit: pageSize, offset: pageIndex * pageSize, q });
      },
      q ? 400 : 0
    );
  ...
  const table = useReactTable({
    ...
    pageCount: Math.max(1, Math.ceil(usersTotal / pageSize)),
    state: {
    ...
    <div className="flex flex-col gap-2 py-4 bg-ghost-white border-1 rounded-md">
      ...
      <div className="flex items-center justify-end space-x-4 px-4 pt-2">
        <div className="flex items-center gap-2">
          <Label className="text-sm text-muted-foreground" htmlFor="admin-users-table-size">
            Rows per page:
          </Label>
          <Select
          ...
          >
            <SelectTrigger id="admin-users-table-size">
        ...
        <Button
          ...
          disabled={pageIndex + 1 >= Math.max(1, Math.ceil(usersTotal / pageSize))}
        >
          Next
        </Button>
      </div>
