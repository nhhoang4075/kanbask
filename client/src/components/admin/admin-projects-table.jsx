...
import { getColumns } from "@/components/admin/admin-project-column";

const PAGE_SIZE_OPTIONS = [10, 20, 50];

export default function AdminProjectsTable() {
  const { projects, projectsTotal, fetchProjects, error } = useAdmin();
  ...
  useEffect(() => {
    const timeout = setTimeout(
      () => {
        fetchProjects({ limit: pageSize, offset: pageIndex * pageSize, q });
      },
      q ? 400 : 0
    );
  ...
  const table = useReactTable({
    ...
    pageCount: Math.max(1, Math.ceil(projectsTotal / pageSize)),
    state: {
    ...
    <div className="flex flex-col gap-2 py-4 bg-ghost-white border-1 rounded-md">
      ...
      <div className="flex items-center justify-end space-x-4 px-4 pt-2">
        <div className="flex items-center gap-2">
          <Label className="text-sm text-muted-foreground" htmlFor="admin-projects-table-size">
            Rows per page:
          </Label>
          <Select
          ...
          >
            <SelectTrigger id="admin-projects-table-size">
        ...
        <Button
          ...
          disabled={pageIndex + 1 >= Math.max(1, Math.ceil(projectsTotal / pageSize))}
        >
          Next
        </Button>
      </div>
