...
import { ListenerProvider } from "@/hooks/use-listener";
    ...
    <SessionProvider>
      <SocketProvider>
        <ChatDataProvider>
          <ListenerProvider>
            {!isCollapsed && (
              <div className="absolute top-19 left-0 h-[calc(100dvh-76px)] w-[100vw] overflow-hidden z-25 bg-black/75"></div>
            )}
            <div className="h-[100dvh] w-full overflow-hidden bg-gray-100 dark:bg-gray-900">
              <SidebarProvider
                defaultOpen={false}
                className="flex flex-col overflow-hidden dark:bg-gray-900"
                style={{ "--sidebar-width-icon": "4rem" }}
              >
                <AppHeader
                  className="fixed flex-none z-25"
