...
export async function PUT(request) {
  try {
    ...
    const data = await fsPromises.readFile(usersFilePath, "utf-8");
    const messages = JSON.parse(data);
    const messagesIndex = messagesNeedChangeId.map((id) =>
      messages.findIndex((message) => message.id === id)
    );
    if (messagesIndex.length === 0)
      return Response.json({ text: "No messages to update", status: 400 });
...
export async function DELETE(request) {
  try {
    ...
    if (deleteMessageIds.length === 0)
      return Response.json({ text: "No messages to delete", status: 400 });
    const newMessages = messages.filter((message) => !deleteMessageIds.includes(message.id));
    if (newMessages.length === messages.length)
      return Response.json({ text: "No messages deleted", status: 400 });
