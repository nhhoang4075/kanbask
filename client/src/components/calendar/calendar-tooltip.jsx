...
export default function CalendarTooltip(info, externalEvent = false) {
  ...
  tooltip.className =
    "fixed max-w-[300px] z-9999 bg-ghost-white rounded-lg shadow-lg px-3 py-2 text-[0.875rem] pointer-events-none";
  ...
  tooltipContent += `
    <div class="flex items-center gap-1 mb-2">
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="${
        info.event.extendedProps.priority === "high"
          ? "text-red-600"
          : info.event.extendedProps.priority === "medium"
          ? "text-amber-500"
          : info.event.extendedProps.priority === "low"
          ? "text-blue-500"
          : "text-gray-600"
      }">
      ...
      <span>Priority: ${
        info.event.extendedProps.priority?.charAt(0).toUpperCase() +
