...
export default function Message() {
  ...
  useEffect(() => {
    ...
    Promise.all([getConversations(), getMessages(), getUsers()])
      .then(async ([conversations, messages, users]) => {
        const setReceivedMessages = messages.map((message) => {
          ...
          if (
            ...
            message.status === "sent" &&
            conversations.some(
              (conv) => conv.participants.includes(currentId) && conv.id === message.conversationId
            )
          ) {
        ...
        const changedMessagesId = setReceivedMessages
          .filter((message) => message.status === "received")
          .map((message) => message.id);
        if (changedMessagesId.length > 0) {
          // Update the messages in the server
          const result = await updateMesssages({
            messagesId: changedMessagesId,
