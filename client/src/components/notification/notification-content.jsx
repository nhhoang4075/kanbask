...
export default function NotificationContent() {
  const { notifications, unreadCount, type, loading } = useNotification();
  ...
  const filteredNotifications = notifications.filter(
    (notification) => notification.reference_type === type || type === "all"
  );
  const unreadNotifications = filteredNotifications.filter((notification) => !notification.is_read);

  return (
    <Tabs defaultValue="all" className="space-y-0 gap-y-0">
