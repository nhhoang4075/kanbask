      ...
      <CardContent className="flex-1 overflow-hidden">
        <div className="h-full overflow-y-auto pr-1 space-y-3">
          {sortedTasks.map(task => (
            <div 
              key={task.id} 
              className={cn("p-3 border rounded transition-colors", hover === task.id && "bg-blue-green text-white")}
            ...
            >
              ...
              <div className={cn("text-xs text-gray-500 transition-colors", hover === task.id && "text-white")}>{projects.find(project => project.id === task.project_id)?.name || "No Project"}</div>
              <div className={cn("text-muted-foreground text-sm mt-1 transition-colors", hover === task.id && "text-white")}>Due: {task.due_date ? new Date(task.due_date).toLocaleDateString("en-UK") : "No due date"}</div>
            </div>
          ))}
        </div>
      </CardContent>
