...
const findUserByEmailPublic = async (email) => {
  try {
    ...
    throw new ApiError(
      ...
      "Lỗi khi tìm kiếm người dùng bằng email."
    );
  }
};
...
const updateProfile = async (userId, updateData) => {
  try {
    ...
    if (!updatedUser) {
      throw new ApiError(StatusCodes.NOT_FOUND, "Không tìm thấy người dùng để cập nhật.");
    }
...
const changePassword = async (userId, oldPassword, newPassword) => {
  try {
    ...
    if (!user || !user.password_hash) {
      throw new ApiError(StatusCodes.NOT_FOUND, "Không thể lấy thông tin mật khẩu hiện tại.");
    }
    ...
    if (!updated) {
      throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Không thể cập nhật mật khẩu.");
    }
    return true;
  } catch (error) {
...
const deleteUser = async (userIdToDelete, requestingUserId, requestingUserRole) => {
  try {
    if (requestingUserRole !== "admin" && userIdToDelete !== requestingUserId) {
      throw new ApiError(StatusCodes.FORBIDDEN, "Không có quyền xóa người dùng này.");
    }
    if (requestingUserRole === "admin" && userIdToDelete === requestingUserId) {
    ...
    if (!deletedUserInfo || !deletedUserInfo.id) {
      throw new ApiError(StatusCodes.NOT_FOUND, "Không tìm thấy người dùng để xóa hoặc lỗi.");
    }
    return true;
  } catch (error) {
...
const updateUserByAdmin = async (adminUserId, userIdToUpdate, updateData) => {
  try {
    ...
    if (userIdToUpdate === adminUserId && dataToUpdate.is_enabled === false) {
      throw new ApiError(StatusCodes.FORBIDDEN, "Admin không thể tự vô hiệu hóa chính mình.");
    }
