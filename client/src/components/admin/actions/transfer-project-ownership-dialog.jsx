...
export default function TransferProjectOwnershipDialog({
  isOpen,
  onOpenChange,
  project,
  listParams
}) {
  const { getProjectDetail, handleTransferProjectOwnership } = useAdmin();
  ...
  const handleSubmit = async (formData) => {
    await handleTransferProjectOwnership(project.id, formData.userId, listParams);
    onOpenChange(false);
  };
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Transfer ownership of {project.name} to another member
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
            id="transfer-project-ownership-form"
        ...
        <DialogFooter className="flex justify-end gap-4 pt-4">
          ...
          <Button
            type="submit"
            form="transfer-project-ownership-form"
